      ...
      <main className="stitch-shell py-12">
        ...
        <section className="mb-16 grid grid-cols-1 gap-8 md:grid-cols-2">
          ...
          <div className="flex flex-col justify-center">
            <span className="mb-2 text-sm font-medium text-ink-600">{locale === 'zh' ? '型号' : 'Model'}: {data.hero.model}</span>
        ...
        <nav aria-label="Product sections" className="mb-8 flex gap-8 overflow-x-auto border-b border-border">
          ...
          <a href="#description" className="shrink-0 pb-4 text-sm text-ink-600 hover:text-ink-950 whitespace-nowrap">{locale === 'zh' ? '产品说明' : 'Description'}</a>
          <a href="#documentation" className="shrink-0 pb-4 text-sm text-ink-600 hover:text-ink-950 whitespace-nowrap">{locale === 'zh' ? '资料下载' : 'Downloads'}</a>
        </nav>
        ...
        <section id="specifications" className="mb-16 grid grid-cols-1 gap-6 md:grid-cols-12">
          <div className="overflow-x-auto border border-border bg-panel md:col-span-8">
            <table className="data-table w-full text-sm text-ink-950">
              <thead>
                <tr><th className="w-1/3">{locale === 'zh' ? '参数' : 'Parameter'}</th><th>{locale === 'zh' ? '数值 / 说明' : 'Value / Description'}</th></tr>
              </thead>
              <tbody>
          ...
          <aside className="border border-border bg-panel p-8 md:col-span-4">
            <h2 className="text-2xl font-medium text-ink-950">{locale === 'zh' ? '主要特点' : 'Key Features'}</h2>
