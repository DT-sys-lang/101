...
module.exports = async (policyContext, _config, { strapi }) => {
  const expectedToken = strapi.config.get('cms-facts.internalToken')

  if (!expectedToken) {
    return deny(policyContext, 'INTERNAL_CMS_FACTS_TOKEN is required.')
  }

  const providedToken = readBearerToken(policyContext)

  if (!providedToken || !safeEqual(providedToken, expectedToken)) {
    return deny(policyContext, 'Invalid CMS facts token.')
  }
